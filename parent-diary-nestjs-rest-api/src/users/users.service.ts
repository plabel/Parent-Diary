...
export class UsersService {
  ...
  generateEncryptedSecretKey(): string {
    ...
    const encryptedSecretKey = AES.encrypt(
      secretKey,
  ...
  decryptSecretKey(encryptedSecretKey: string): string {
    const secretKeyRaw = AES.decrypt(
      encryptedSecretKey,
      this.configService.get('master_key'),
    ).toString(enc.Utf8);
  ...
  async logIn(logInDto: LogInDto): Promise<User | null> {
    ...
    if (
      ...
      this.hashPassword(logInDto.password, user.dataValues.salt)
    ) {
      return null;
    }
    ...
    if (!isOTPValid) {
      const isRecoveryCodeValid =
        user.dataValues.recoveryCode === logInDto.recoveryCode;
  ...
  async resetPassword(token: string, password: string): Promise<boolean> {
    ...
    const decryptedToken = Buffer.from(
      buffer.toString(enc.Utf8),
      'hex',
    ).toString();
    const tokenObj = JSON.parse(decryptedToken);
    if (
      tokenObj.creationDate <
      Date.now() -
        1000 * 60 * this.configService.get('token_expiration_time_in_minutes')
    ) {

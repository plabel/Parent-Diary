...
export const testCases: SendResetPasswordEmailTestCase[] = [
  ...
  {
    description: "should not send reset password email, if fetchWrapper returns an error",
    showAlertArgs: ["danger", "Reset password email failed"],
    fetchWrapperMockResult: { data: false, error: { message: "Reset password email failed" } },
    formData: (() => {
      const formData = new FormData();
      formData.set("email", "test@test.com");
      return formData;
